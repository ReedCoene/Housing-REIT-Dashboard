"""
Housing REIT Dashboard — Daily Data Fetcher
...
REITS = {
    ...
    # Manufactured / lifestyle housing
    "SUI":  "Sun Communities",
...
# ── Broad housing / real estate RSS feeds ─────────────────────
BROAD_FEEDS = [
    {"source": "GlobeSt Multifamily",   "url": "https://www.globest.com/category/multifamily/feed/",         "category": "sector"},
    {"source": "GlobeSt Residential",   "url": "https://www.globest.com/category/residential/feed/",         "category": "sector"},
    {"source": "HousingWire",           "url": "https://www.housingwire.com/feed/",                          "category": "broad"},
    {"source": "Multifamily Executive", "url": "https://www.multifamilyexecutive.com/rss/",                  "category": "sector"},
...
SIGNAL_KEYWORDS = [
    ...
    "rent growth", "leasing", "vacancy", "cap rate", "noi", "same-store",
    "affordability", "supply", "demand", "new construction", "permits",
]
...
def fetch_newsapi() -> list:
    ...
    queries = [
        "multifamily REIT apartment rent",
        "single family rental housing real estate",
        "AvalonBay OR Equity Residential OR Invitation Homes OR Sun Communities",
    ]

    HOUSING_TERMS = [
        "apartment", "multifamily", "rental", "housing", "reit", "rent",
        "occupancy", "vacancy", "lease", "tenant", "residential",
        "affordability", "single family", "manufactured", "cap rate",
    ]

    def is_relevant(art):
        text = ((art.get("title") or "") + " " + (art.get("description") or "")).lower()
        return any(t in text for t in HOUSING_TERMS)
...
FOCUS_DETAILS = {
    ...
    "thesis_points": [
        "Update this section with your MRP investment thesis",
        "Key catalysts, competitive positioning, and risk factors",
        "Management track record and capital allocation strategy",
        "Valuation vs. peers — implied cap rate, NAV discount/premium",
    ],
}


def main():
    print("=== Housing REIT Dashboard Data Fetch ===")
