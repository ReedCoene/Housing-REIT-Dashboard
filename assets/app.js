...
function renderFocus(data) {
  ...
  document.getElementById('focusHero').innerHTML = `
    <div class="ctre-hero">
      <div class="ctre-hero-left">
        <h2>MRP Group &nbsp;&mdash;&nbsp; NYSE: MRP</h2>
  ...
  const ctreNews = (data.news || []).filter(n => n.ticker === 'MRP');
  ...
  ctreEl.innerHTML = ctreNews.map(n => `
    <div class="news-card" style="margin-bottom:8px">
      <div class="card-meta">
        <span class="card-source ${n.is_signal ? 'signal-src' : ''}">MRP</span>
...
function renderWeeklyReport(data) {
  ...
  el.innerHTML = `
    ...
    <div class="section">
      <div class="section-header"><h2>Housing &amp; Macro Highlights</h2></div>
      <div class="panel">
...
function renderBrief(data) {
  ...
  const sentimentText = majority === 'up'
    ? `${advancing} of ${stocks.length} housing REITs advanced today`
    : majority === 'down'
    ? `${declining} of ${stocks.length} housing REITs declined today`
    : `Housing REITs were mixed today`;
  ...
  document.getElementById('todaysBrief').innerHTML = `
    <div class="brief-wrap">
      ...
      <div class="brief-sections">
        ...
        ${macro.length ? `
        <div class="brief-section">
          <div class="brief-section-label">Housing &amp; Policy</div>
